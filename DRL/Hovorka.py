...
def hovorka_model(t, x, u, D, P):  ## This is the ode version
    ...
    S1 = x[2]  # Amount of insulin absorption in compartment 1 [mU]
    S2 = x[3]  # Amount of insulin absorption in compartment 2 [mU]
...
def run_simulation():
    ...
    init_basal = 6.43                           # 初始的胰岛素基础速率
    P = hovorka_parameters(70)                  # 体重为70KG的人的参数
    initial_pars = (init_basal, 0, P)           # 将参数打包，分别为初始胰岛素基础速率，碳水化合物摄入，其他参数

    # Bolus carb factor -- [g/U]
    carb_factor = 25

    # Initial value
    # 求解非线性方程组,参数分别为要求解的方程组，初始值，方程组参数
    ...
    for i in range(1440):
        ...
        insulin_rate_curve.append(insulin_rate)
        blood_glucose_value.append(int(integrator.y[-1] * 18))

    # Returning blood glucose value
    return blood_glucose_value,insulin_rate_curve
...
if __name__ == "__main__":
    ...
    for i in range(len(meals)):
        plt.vlines(meal_times[i],0,meals[i],colors='red')
        plt.annotate(r'摄入碳水量%d'% meals[i],xy=[meal_times[i],meals[i]],
                     xycoords='data',xytext=(meal_times[i]-100,meals[i]-20))
    plt.title('模型血糖变化曲线')
    plt.show()
    # 绘制胰岛素曲线图
    plt.figure()
