    ...
    def save_itr_params(self, itr, params):
        if self._snapshot_dir:
            if self._snapshot_mode == 'all':
                ...
                # print(self._snapshot_gap)
                if isinstance(params['eval_policy'],ArgmaxDiscretePolicy):
                    if itr % self._snapshot_gap == 0:
                        orig_run_device = params['qf'].device
                        cache_file_name = osp.join(self._snapshot_dir, 'cache_itr_%d.pkl' % itr)
                        qf_cache = params['qf'].cpu().state_dict()
                        target_qf_cache = params['target_qf'].cpu().state_dict()
                        joblib.dump(
                            {'qf': qf_cache,  'target_qf': target_qf_cache,
                             },
                            cache_file_name)
                        optim_file_name = osp.join(self._snapshot_dir, 'optim_itr_%d.pkl' % itr)
                        qf_optim = params['qf_optim'].state_dict()

                        joblib.dump({'qf_optim': qf_optim,}, optim_file_name)
                        params['qf'].to(orig_run_device)
                        params['target_qf'].to(orig_run_device)
                    ...
                    cache_file_name = osp.join(self._snapshot_dir, 'cache_itr_%d.pkl' % itr)
                    qf_cache = params['qf'].cpu().state_dict()
                    target_vf_cache = params['target_qf'].cpu().state_dict()
                    joblib.dump({'qf': qf_cache, 'target_qf': target_vf_cache,
                                 },
                                cache_file_name)
                    optim_file_name = osp.join(self._snapshot_dir, 'optim_itr_%d.pkl' % itr)
                    qf_optim = params['qf_optim'].state_dict()
