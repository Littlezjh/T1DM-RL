...
class RLAlgorithm(metaclass=abc.ABCMeta):
    ...
    def _take_step_in_env(self, observation):
        ...
        if self.action_map is None:
            ...
            next_ob, raw_reward, terminal, env_info = (
                self.training_env.step(self.action_map[action])
            )
    ...
    def _try_to_eval(self, epoch, eval_paths=None):
        ...
        if self._can_evaluate():
            self.evaluate(epoch, eval_paths=eval_paths)         # 使用当前时刻训练处的policy来和模拟器进行交互

            params = self.get_epoch_snapshot(epoch)
            logger.save_itr_params(epoch, params)       # 保存模型的参数
    ...
    def _can_train(self):
        return (
            self.replay_buffer.num_steps_can_sample() >=        # 获取replay_buffer内的数据总量，是否大于等于最小训练样本数
    ...
    def evaluate(self, epoch, eval_paths=None):
        ...
        if eval_paths:
            ...
            test_paths = self.get_eval_paths()      # 使用epoch结束后的policy和环境进行交互，使用10天的长度来验证效果
        statistics.update(eval_util.get_generic_path_information(           # 获得在10天的测试长度上的Reward
    ...
    def get_eval_paths(self):
        return self.eval_sampler.obtain_samples(self.action_map)
