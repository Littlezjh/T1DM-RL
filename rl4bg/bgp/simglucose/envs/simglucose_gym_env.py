...
class DeepSACT1DEnv(gym.Env):
    ...
    def __init__(self, reward_fun, patient_name=None, seeds=None,
                 ...
                 custom_meal_size=1, starting_glucose=None,discrete=False,
        ...
        self.control_quest = '{}/bgp/simglucose/params/Quest2.csv'.format(self.source_dir)                  # 包含CR，CF，年龄等信息
        ...
        self.seeds = seeds
        self.sample_time = 5
        self.day = int(1440 / self.sample_time)     # day=288
        self.state_hist = int((n_hours * 60) / self.sample_time)    # n_hours表达收集最近多少小时的数据作为状态
        self.time = time
        self.meal = meal
        self.norm = norm
        self.gt = gt
        ...
        self.set_patient_dependent_values(patient_name)     # 初始化病人参数，CGM对象，饮食表，胰岛素泵对象等。
    ...
    def _step(self, action, cho=None, use_action_scale=True):
        ...
        _, reward, _, info = self.env.step(act, reward_fun=self.reward_fun, cho=cho)    # 获取reward和其他信息
        state = self.get_state(self.norm)
        done = self.is_done()

        if done and self.termination_penalty is not None:       # 如果结束了，那么reward-1e5作为惩罚
    ...
    def get_state(self, normalize=False):
        ...
        if self.gt:
            ...
            return state
        return np.stack(return_arr).flatten()       # 将288个血糖和胰岛素数据平铺
    ...
    def set_patient_dependent_values(self, patient_name):
        ...
        vpatient_params = pd.read_csv(self.patient_para_file,engine='python')
        quest = pd.read_csv(self.control_quest,engine='python')
        self.kind = self.patient_name.split('#')[0]         # 患者是成人，青少年还是儿童
        ...
        pid_df = pd.read_csv(self.pid_para_file,engine='python')
        ...
        patient = T1DPatientNew.withName(patient_name, self.patient_para_file)                      # 建立对应病人数据的对象
        sensor = CGMSensor.withName('Dexcom', self.sensor_para_file, seed=self.seeds['sensor'])     # 建立CGM传感器对象
        if self.time_std is None:       # 如果时间变化方差为0
        ...
        elif self.use_custom_meal:      # 使用自定义的饮食时间表
            ...
            scenario = SemiRandomBalancedScenario(bw=self.bw, start_time=self.start_time, seed=self.seeds['scenario'],
                                                  ...
                                                  meal_duration=self.meal_duration)
        pump = InsulinPump.withName('Insulet', self.insulin_pump_para_file)                         # 建立胰岛素泵对象
        self.env = T1DSimEnv(patient=patient,
                             sensor=sensor,
                             pump=pump,
                             scenario=scenario,
                             sample_time=self.sample_time, source_dir=self.source_dir)              # 使用env中的T1DSimEnv创建实际执行模拟操作的对象
        if self.hist_init:
            # 是否初始化历史记录
